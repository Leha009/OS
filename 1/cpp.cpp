...
void FileAttributesActions()
{
    ...
    FileTimeToSystemTime(&ftToLocal, &systemTime);
    cout << "\nFile last access time: " << GetFullDate(&systemTime) << '\n';
    FileTimeToLocalFileTime(&lastWriteTime, &ftToLocal);
    FileTimeToSystemTime(&ftToLocal, &systemTime);
    cout << "\nFile last write time: " << GetFullDate(&systemTime) << '\n';
    CloseHandle(hFile);
}
...
string GetFullDate(LPSYSTEMTIME systemTime)
{
    ...
    ssFullDate << systemTime->wYear << " at ";
